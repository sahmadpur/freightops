/**
 * Azerbaijani country names, keyed by ISO 3166-1 alpha-2 code.
 *
 * Why this table exists: Chromium ships no Azerbaijani region data, so
 * `Intl.DisplayNames(["az"], { type: "region" })` silently falls back to
 * English in the browser while Node's full ICU returns real Azerbaijani. Any
 * component that localizes a country name on both sides of hydration then
 * renders two different strings — a hydration mismatch, and English country
 * names for AZ users. Reading from this table makes the output identical in
 * every runtime.
 *
 * Generated from Node 24's ICU (`Intl.DisplayNames(["az"], {type:"region"})`)
 * over the code list in `countries.ts`. Regenerate the same way if that list
 * grows.
 */
export const AZ_REGION_NAMES: Record<string, string> = {
  AD: "Andorra",
