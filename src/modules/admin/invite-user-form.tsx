...
export function InviteUserForm({ accounts }: { accounts: { id: string; title: string }[] }) {
  ...
  return (
    <div className="flex flex-wrap items-end gap-2">
      <div>
        <label className="mb-1 block text-[11.5px] font-medium text-ink-soft">{t("email")}</label>
      ...
      {role === "client" && (
        <div>
          <label className="mb-1 block text-[11.5px] font-medium text-ink-soft">{t("account")}</label>
